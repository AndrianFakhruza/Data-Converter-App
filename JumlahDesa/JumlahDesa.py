...
class InputDataDesa(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Input Data Desa')
    ...
    def setup_ui(self):
        ...
        title_label = QLabel("Input Data Desa")
        ...
        self.nama_desa_list = [
            "BL. TALON", "SIDO MULYO", "LHOKJOK", "BAYU", "PANTON RAYEK 1", "MEUNASAH DAYAH",
            "FASKES LAIN", "KR. SENONG", "COT MERBO", "BUKET", "MC. BAHAGIA", "MEUNASAH KUMBANG",
            "CEMPEDAK", "COT RHEU", "CEMECET", "PULO BARAT", "BL. GURAH", "COT SEUTUI", "BABAH LUENG",
            "KRESEK", "KD. KRUENG", "MULING MANYANG", "BL. ARA", "BL. ADO", "MEURIYA", "KR. MANYANG",
            "KEUDE BLANG ARA", "COT SEMIYONG", "MULING MEUCAT", "ALU RAMBE", "MEUNASAH KULAM",
            "SEUNEBOK DRIEN", "BL. RIEK", "KR. SEPENG", "PULO IBOH", "LANGKUTA", "PULO RAYEUK",
            "P. RAYEUK 2", "GUHA ULHEU", "SAWEUK"
        ]

        self.nama_desa_list = sorted([nama.upper() for nama in self.nama_desa_list])

        self.labels = []
        self.line_edits = []

        for nama_desa in self.nama_desa_list:
            label = QLabel(nama_desa)
    ...
    def submit_data(self):
        ...
        for label, line_edit in zip(self.labels, self.line_edits):
            nama_desa = label.text()
            jumlah = line_edit.text()
            if jumlah.isdigit():
                data.append({'Nama Desa': nama_desa, 'Jumlah': int(jumlah)})
            else:
                QMessageBox.warning(self, 'Input Error', f'Invalid input for {nama_desa}. Please enter a valid number.')
                return  

        if data:
            selected_month = self.month_combo_box.currentText().upper()
            self.simpan_ke_excel(data, f'GRAFIK DATA JUMLAH PEMERIKSAAN LABORATORIUM PERDESA BULAN {selected_month} 2024.xlsx')
            self.buat_grafik(data, f'GRAFIK DATA JUMLAH PEMERIKSAAN LABORATORIUM PERDESA BULAN {selected_month} 2024', f'GRAFIK DATA JUMLAH PEMERIKSAAN LABORATORIUM PERDESA BULAN {selected_month} 2024.png')
    ...
    def buat_grafik(self, data, title, nama_file):
        ...
        bars = plt.bar(df['Nama Desa'], df['Jumlah'], color='skyblue')
